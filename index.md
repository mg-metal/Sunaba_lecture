...
- 上で紹介した『 引数に最大値`max`を取り、戻り値として `0` から `max` の範囲内の乱数を返す 』 という関数を作りたい人は各自チャレンジしてほしい。考える時のヒントを記しておく
    ...
    - 線形合同法で生み出した素の値は、線形合同法のコードだけで更新する。乱数がほしいプログラマーは、その都度ほしい範囲を指示して抜き出していく、というイメージ。
    - Sunabaでは関数内に値を保持する変数を作れない（C言語の静的変数, static変数）が、直接にメモリ番号指示でアクセスすることは可能である。つまり、線形合同法で生み出した素の値は、関数の外側に確保しておく領域を用意すればいい
