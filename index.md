# Sunaba講座
## 本講座の対象
- プログラミング学習経験が無い、もしくは無いに等しいのに、C言語習得を迫られている人
- 黒い画面に文字を表示するだけのプログラミングに、やる気を無くした人

## 対象外の人
- 何かしらのプログラミング言語を学習し、黒画面＆文字の世界に楽しさを見い出せた人
- 時間がたっぷりあり、お手本プログラムを写しながら徐々に慣れていく方法を採れる人
- [書籍『 プログラムはこうして作られる - プログラマの頭の中をのぞいてみよう 』](https://www.amazon.co.jp/dp/479803925X) で学習できそうな人（まずはそちらにチャレンジ！）

## 何のためにプログラミングを勉強するのか？
- できれば `何のためにプログラムを学ぶか` を意識して、ここでの学習が役立ちそうか否かを判断してもらいたい
- その目的意識が `とりあえずやってみたい！` であってもいいと思う
- 本内容はSunaba卒業後にC言語を学ぶ前提で話を進める。とはいえ、C言語を学ぶ予定がなくてもSunabaが楽しいと思える人であれば使えると思う

## Sunabaを手に入れる
- Sunaba本体
    - [Sunaba公式ページ](https://hirasho.github.io/Sunaba/) のダウンロード(windows or mac) をクリック 
    - Sunaba.exe をダブルクリックして、Sunabaウィンドウが起動するかどうかを確認
        - Sunabaが起動せず `.NET Framework*******`というメッセージが表示される場合がある。その場合は指示に従い必要なものをダウンロード＆インストールすること
- その他資料
    - Sunaba本体以外の資料を入手してほしいので、[保管場所１](https://onedrive.live.com/?id=FEB9AEFDFB7E311%21136&cid=0FEB9AEFDFB7E311) にアクセス
        - Sunaba160413.zip と Sunaba追加パッケージ160413.zip をダウンロードする
        - 注意：Sunaba160413.zip 内のSunaba.exeは旧Ver.なので使わないこと


## Sunabaってなに？
- 教育用のプログラミング言語である  
- まずは実際に遊んでみよう
    - サンプルフォルダ内の`お手本.txt`をSunabaウィンドウにドラッグ＆ドロップ
...
- （研修では）『sunaba_氏名』というフォルダを各自で作成
    - 課題ごとに作成したプログラムファイルを格納するためのフォルダである
    - 指示があり次第、提出エリア（クラウド領域 or ローカル共有PC）にフォルダごとアップロードする
...
- 65050番のメモリのダイヤルを 999999 に合わせるよ という意味
    - メモリの中は空っぽ状態にならないぞ、と理解してもらうための比喩である
    - C言語の書籍ではメモリを `箱` に例えることが多いが、それがメモリの理解を邪魔するケースがある
    - とはいえ私は、口答で解説する時にメモリを `箱` に例えることが多く、その習慣で本テキストでも `箱` で表現する場面があるかもしれない
    - 柔軟に受け止めてほしい。。『`空っぽにならない箱` なんだろうねぇ』という感じで。

---
### メモリなのに画面表示？
- メモリには種類がある
- 0～39999番は一般的な記憶領域。数値を記憶するだけ。***<font color = 'orange'>読み書き可</font>***
    - ただし、39999から遡るように作成したプログラム自体が格納される。注意点は後述する
...
- まずは解説を読まずにチャレンジ
    ...
    - 未経験者は`100行プログラム`を書いてみよう
        - コピペを活用すること!
- これ以降は、一つに課題につき複数のプログラムを作る可能性が高い。ファイル名に追番をつけて `kadai02_01.txt` とする
...
- 式の構成要素
    - 数値：整数のみ。小数は不可。使用可能範囲はおよそ －20億 から ＋20億（使用場面により範囲は変動する）
    - メモリ：メモリ0番～30000番台に格納できる数値も －20億 から ＋20億。他の番地は役割毎に扱う数の範囲は異なる（早見表.txt参照）
        - 【注意】39999番から遡るようにプログラム自体が格納されるため、30000番台は直接読み書きしないようにしよう（少なくとも今は）
    - メモリの使い方には禁止事項がある
...
- Sunaba早見表を見ながら、試行錯誤で形にできそうな人はそうしよう。作業を終えたら戻ってきてほしい
- Sunaba早見表を見てむずかしいと感じる人は、今回も画面メモリ番号直打ちの１００行プログラムを書こう。ただしコピペを活用すること。実行結果を確認できたら戻ってきてほしい
...
1. 最終形の全体像
    - 文章だけで解説することが難しいので、全体像を疑似コードとコメントで提示したい
...
1. 演算処理の優先順位
    - 上の擬似コードの `点描画  # 横線長さカウンタと縦長さカウンタで構成した式` は作れただろうか？
    - 答えを言ってしまうと、`60000 + 縦カウンタ×100 + 横カウンタ` で画面メモリ番号を指示できる。しかし、Sunaba特有の問題がある
    <details><summary>問題ありVer.</summary>
    <pre><code>
...
---
## **課題09** 大きな四角を描く
- `課題08の四角を、幅 60、高さ 80 に変更せよ。` 
- 大きな絵でも一瞬で表示できるよね、を確認するだけの課題。

---
---
## **課題10** たくさんの点を描く（変数を使う）
- `複数個の点を描く。個数、色、位置は任意である。ただし各パラメータは変数に格納して描画すること`  
...
- メモリ0番～30000番台であれば、関数の内側と外側で同じデータを共有できることになる
    - しつこいが、30000番台は後ろの方からプログラム自体が格納されるので、データ管理用のメモリに30000番台は使わないほうがいい
...
- Sunabaのメモリ0番～30000番台 の管理・運用方法に正解はない。
    ...
    - Sunaba講座の意義は、論理を組み立てる過程に集中してもらうことにある。`平たく言えば、粘り強く考えてほしいだけある。`成果物の客観的な技術レベルよりも、動くものを作れた時の各々の充実感の方が重要
        - <font color="Silver">後日のC言語講座は、職場で使うことを想定しているので、頭を切り替えてもらう必要がある</font>  
    
#### 特徴５：関数内で計算した結果をひとつだけ、メイン側に返すことができる（戻り値、返り値）
- 今回の課題には不要な機能だが、関数の特徴として最後の項目となるので、ここで伝えておきたい
...
---
## **課題11 横線を描く（関数）**
- `横線を描く関数を作る。入力は4つ：左端点座標x,y 、 長さlen(Lengthのlen) 、色col(Colorのcol) `
...
- 関数化は後回しにして、まずは、これを x, y, len, col で表現しよう
...
- 【新情報 2】複数の判定結果の判断 `または(or)` と `かつ(and)`
    ...
    - そもそも `xが0～99の範囲内` は判定式としてどうやって書けばよいか？
        ...
        - C言語には判定式の結果をつなげる `論理演算子` がある
            - または(or) ： `||`
            - かつ(and) ： `&&`
            - 等々
        - しかし、Sunabaには論理演算子がない
        - 判定式の結果が 1（真）と 0（偽）という整数であることに注目        
        - 普通の算術演算子を使って論理演算が可能である
            - または(or) ： `+` （足し算）
                - 1 + 1 → 2（真）
                    - 0以外は真である
                - 1 + 0 → 1（真）
            ...
            - かつ(and) ： `*` （掛け算）
                ...
                - 0 * 1 → 0（偽）
                - 0 * 0 → 0（偽）
        - 2つの判定式を合わせる際、`0 <= x * x <= 99` はもちろんNGだ。Sunabaには演算子の優先順位はない
        - xの判定式 ： `(0 <= x) * (x <= 99)` 
        - y も全く同じなので、
        - yの判定式 ： `(0 <= y) * (y <= 99)`
        - 本課題の判定式の最終形
...
- 上で紹介した『 引数に最大値`max`を取り、戻り値として `0` から `max` の範囲内の乱数を返す 』 という関数を作りたい人は各自チャレンジしてほしい。考える時のヒントを記しておく
    - 今回コピペした線形合同法の関数は、自分が生み出した乱数を使って、次の乱数を生み出す、という仕組みである
    - 線形合同法で生み出した素の値は、線形合同法のコードだけで更新して、ある範囲の乱数がほしいプログラマーは、その都度ほしい範囲を指示して抜き出していく、というイメージで考えてみよう
    - Sunabaでは関数内に値を保持する変数を作れない（C言語の静的変数, static変数）が、直接にメモリ番号指示でアクセスすることは可能である。線形合同法で生み出した素の値は、関数の外側に確保しておく領域を用意すればいい
...
- まずは自力で！
    ...
    while 1
        ...
        # 描画
        point(x, y, color)
        rect(rc_x, rc_y, rc_w, rc_h, rc_color)
...
- 自機は、左右移動のみ出来る横長（5ドット）の棒で、画面の下の方に位置させる
- 弾は１ドットの点
...
- オブジェクトと変数の関係を整理できていないうちに使うと、読解できないスパゲッティプログラムになるので注意
    - 関係性を整理するには `変数名で役割を明確にしよう`。どのオブジェクトの、何を意味する変数なのかを変数名で表現する
...
- 上述の手法を要約すると `オブジェクトが持つ変数（例えば自機の座標x,y）をグローバル変数にし、関数内から直接読みに行けるようにすることで大きなプログラムを整理する方法` となる
    - `グローバル変数` とはどこからでもアクセスできる変数。詳しくはWeb検索で調べてみよう
- 大前提として、この方法を実際の仕事で使ってはいけない。間違いなく怒られる。そのことを理解してもらったうえで話を進める
- これからSunaba後のC言語学習の流れを示すのだが、まずはSunabaよりも不便な環境が過去にあったことを話したい
- かなり前に普及していた `BASIC` という言語にはスコープがなかった。つまり前述の手法はSunaba特有の話ではないことになる（今のBASICにはスコープがある）
- しかも、BASICには変数名に大文字・小文字の区別がつかないので、整理の難しさについてはSunabaよりも厳しい（BASICは大文字だけで書くのが普通）
- そのBASICで複雑なPCゲームを作っていた時の厄介さに比べれば、冒頭の手法でSunabaコードを書くことは難しい話ではないかもしれない。しかし、それは昔との比較であって、まあまあ無茶な作業であることに変わりない
...
- 自機クラスのx,y座標は、自機クラスがもつmove関数の中からグローバル変数にアクセスする様に書くことが可能（引数で受け取る必要がないということ）。それでいて、他のオブジェクトから読み書きされる危険を排除する仕組みなのである
...
- `C言語のファイル分割` は、`オブジェクトごとに世界を区切る` ために実施するものである。長くなったプログラムを分割するのが主な理由ではない
    - だから、複数ファイルの中で異様に短いプログラムがあっても別におかしくないのである
- この`C言語のファイル分割` の意義を知るためには、C言語を学んでいる最中に他のプログラミング言語でクラスの初歩を学習するべきだと考えている
- C言語のファイル分割は、はっきり言って直観的でない。`オブジェクトの変数・関数は独立していると良い` という感覚的に理解できることを、直観的に使えない道具 `ファイル分割` で実現しなければならない
- だからC言語のみで `オブジェクトの変数や関数を独立させる考え方` を理解しようとするよりも、他のプログラミング言語のチカラを借りたほうが良いと考えるのである
- 私はその役割として `Proessing` が一番だと思う。理由は以下の通り
    - 文法がC言語に近いので学習時間を短くできる。それから、いわゆる `おまじない` が排除されているので、`「これどういう意味？」`を意識せずに済む
    - 実行結果を動きのある絵で示せる
    - トライ＆エラーが容易（開発環境のエディタに直接プログラムを書いて実行ボタンを押すだけ）
    - 【注意】`Proessing`はJavaの文法が使えるが、オブジェクトの独立性をコントロールするアクセス修飾子(private等) は機能しない

- C言語学習の前にProcessingを利用しないのは、
    - `便利な道具に慣れ親しんだ後に、わざわざ不便なものに時間を割きたくなるか？` という懸念（例外的な人がいるのはわかるが、ここでは多くの人に当てはまることを考えたい）
    - C言語と同様、インデントを付けなくても動作には影響しないので、Processing初心者はインデントが崩れたままゴチャゴチャ書いてしまうケースをよく見る。学習時間が豊富であれば目くじら立てないが、C言語習得の時間が限られている状況でこの回り道は痛い
- C言語学習前にSunabaを利用するのは、
    - 黒い画面とのやり取りに面白さを見いだせない人に、違う面白さを提供できる
        - C言語であってもDXライブラリを使えば学習しやすい。参考文献が多いので。しかし実行までの待ち時間が長い
    - 文法学習を最小化することで作品作りに意識を持っていきやすい。早いうちにプロジェクトベースの訓練に移行できる
        - C言語は文法ルールが少ない言語だけど、初心者にとっては規模が大きい
    - インデントをつける習慣を持っている状態でスタートを切れる
        - C言語へ移行したばかりの時は中括弧 `{...}` が煩わしく思えるかもしれない
    - 文字の入出力と外部ファイルアクセスできることが嬉しいと思える。便利かつ世界が広がった印象を受ける
        - もしかしたら黒い画面で文字だけのやり取りをするプログラムに面白みを感じられるかも
- ちなみにC言語の文字操作とファイル操作は他の言語に比べて面倒くさい。C言語だけで使いこなす域を目指すより、ここは他言語の楽さ加減を知っておいていいかもしれない。ここでもProessingを使えばいい
...
- 連打ゲーム
    ...
    - 複数対戦を作りやすいのも特徴
    - ボタン入力に工夫がいる。キー入力メモリを読むだけのプログラムでは押しっぱなしが最強になる。押した瞬間だけ入力をカウントする仕組みが必要。
- pong 
