...
- 過去のプログラミング学習で、黒い画面に文字を表示するだけの環境にやる気を無くした人
...
- その他資料
    - Sunaba本体以外の資料を入手するため以下のURLにアクセス
    - [保管場所１](https://onedrive.live.com/?id=FEB9AEFDFB7E311%21136&cid=0FEB9AEFDFB7E311)
        - Sunaba160413.zip と Sunaba追加パッケージ160413.zip をダウンロードする
        - 注意：Sunaba160413.zip 内のSunaba.exeは旧Ver.なので使わないこと
- Sunaba.exe 実行時、`.NET Framework*******`というメッセージが表示される場合がある。その場合は指示に従い必要なものをダウンロード＆インストールすること

## Sunabaってなに？
- 教育用のプログラミング言語である  
- まずは実際に遊んでみよう
    - サンプルフォルダ内の`お手本.txt`をSunabaウィンドウにドラッグアンドドロップ
...
- （研修では）『sunaba_氏名』というフォルダを各自で作成
    ...
    - 指示があり次第、提出フォルダにアップロードする（クラウド領域 or ローカル共有PC）
...
- `memory[...]` の `...` は、メモリの番号
- Sunabaのメモリはダイヤル式
    - 空っぽの状態にはならない、と言いたいだけ
    - 口答で解説する時にメモリの例えがダイヤルではなく `箱` になることがある。その際は脳内変換してください。。。
