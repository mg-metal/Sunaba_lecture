...
## Sunabaってなに？
- 教育用のプログラミング言語
    - 開発環境の解説は、ビデオ通話の画面共有で実施
- とりあえずお手本（テトリス）を見てみましょう
## Sunabaの特徴
- 覚えなきゃならない約束事が少ない
- 大抵の機能は自分で作らないといけない
...
## 何のためにプログラミングを勉強するのか？
- （我々の）業務への繋がりやすさとしては、
    - C言語でマイコンを操作できるようになるため
    - ExcelVBAでマクロを組めるようになるため
- たまに C++、C# スキルを必要とする事例はあるが、4月研修はそこまで視野を広げない
    - 5月中旬以降の『自由課題』はどの言語を選択してもOK
- 業務関係なしでもゲーム制作やマイコン工作ができれば楽しいでしょ？
## はじめからC言語やVBAでいいじゃん
- 大丈夫な人はSunabaをやる必要ない
- 大丈夫かどうかは、やってもらわないとわからないので全員強制参加です
- Sunabaは日本語でもプログラミングできるけど、、、
    - ExcelVBAは日本語OKだが、C言語・Arduino・VerilogHDL等はNG
- プログラミング未経験者も最初から英語でいこう
...
1. 新しいテクニックやルールの紹介
    ...
    - 判定式の演算結果が **<font color = 'yellow'>真</font>** な限り、「while文の中のプログラム」を繰り返し実行する
    - 判定式の演算結果が **<font color = 'skyblue'>偽</font>** なら、「while文の中のプログラム」を飛ばして、下に続くプログラムに移動する
        - **<font color = 'yellow'>真</font>** **<font color = 'skyblue'>偽</font>** を端的にいえば
            - 真：演算結果が０以外
...
- チャレンジャーなら、まずは解説を無視してコードを書いてみましょう
    - **<font color = 'pink'>【横道に逸れるけど大事なこと】</font>**
    ...
    - 『自分、そういうところあるかも』という人は気をつけよう（講師も気をつけます）
    - 何事もバランスが大切です
...
---
### 課題04の解説
- 画面更新の話が長くて、課題の内容が飛んだと思うので再掲しよう
...
- そろそろ `memory[....]` だらけのプログラムに嫌気がさしてきたのではないだろうか
    ...
    -  `memory[....]` を `m[....]` に短縮する方法
        - <font color = "gray">（以下の解説、現時点で分からなくてもいい。『コードの冒頭で`const m -> 0` を書けば `m[....]` でメモリ操作できる』それだけ理解してくれればOK）</font>
...
- <font color="gray">【補足】実は、、
    - 課題08のような無限ループの無いプログラムでは、手動更新モードに設定している場合、画面更新指示がなくても表示出来てしまう。
    - これは手動更新モードの仕組みによるもの。プログラムが終了する時に、画面メモリの最終的な値を画面に出力するようになっている（Sunabaの仕様）
    </font>
...
- プログラム序盤に`変数宣言`を行ない、その下に、手動更新モード切替、プログラム本編の開始、という流れで書く
    ...
    - <font color="gray">後日学ぶC言語では { } で囲われたブロックにおいて、その { } 内の冒頭で変数宣言をしないとエラーになる
        - C99以降の規格はエラーにならないが、開発環境によって古い規格が生きており、エラーになる可能性がある
        - 宣言場所を問わない環境であっても、ブロックの冒頭で変数宣言するのが苦にならないくらい1ブロックを小さくする。読みやすいコードになると思う（狙いが明確で、問題ないことも確認済なら新しいルールを使おう）
        </font>
